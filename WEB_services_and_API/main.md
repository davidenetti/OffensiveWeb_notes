...
*BOLA is the same as IDOR*.

Example scenario:
We have the credential of a e-commerce. The admin of it wants from us an assessment on the APIs.
We find out that we can see data from other users and not the logged in one.
...
## Improper sanitization of SQL commands
